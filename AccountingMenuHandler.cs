...
namespace Smart_Factory_Management_System
{
    internal static class AccountingMenuHandler
    {
        ...
        private static void ShowBatches(Factory factory)
        {
            ...
            var table = new Table().AddColumn("BatchId").AddColumn("Product").AddColumn("Qty").AddColumn("UnitCost").AddColumn("UnitSell").AddColumn("Sold");
            for (int i = 0; i < factory.BatchCount; i++)
            {
                var b = factory.Batches[i];
                table.AddRow(b.BatchId, b.ProductName, b.Quantity.ToString(), b.UnitProductionCost.ToString("F2"), b.UnitSellPrice?.ToString("F2") ?? "-", b.IsSold ? "Yes" : "No");
            }
        ...
        private static void SetBatchPrice(Factory factory)
        {
            ...
            double price = AnsiConsole.Ask<double>("Set unit selling price for this batch ($):");
